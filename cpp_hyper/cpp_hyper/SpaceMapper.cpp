...
//     SpaceMapper mapper = SpaceMapper::Uncorrellated_Space(X);
//     Eigen::MatrixXd p = mapper.project(X);
//     Eigen::MatrixXd up = mapper.unproject(p);

//     std::cout << "Projected:\n" << p << "\n";
//     std::cout << "Unprojected:\n" << up << "\n";

//     int dim = SpaceMapper::PCA_Dimension(X);
//     std::cout << "PCA dimension: " << dim << "\n";
...
SpaceMapper::SpaceMapper() : stop_s(0) {}

Eigen::MatrixXd SpaceMapper::project(const Eigen::MatrixXd &correllated_poses) const {
    Eigen::MatrixXd centered = correllated_poses.rowwise() - Xavg_;
    Eigen::MatrixXd proj = centered * V_.topRows(stop_s).transpose();
...
Eigen::MatrixXd SpaceMapper::unproject(const Eigen::MatrixXd &uncorrellated_poses) const {
    Eigen::MatrixXd input = uncorrellated_poses;
    if (scale.size() > 0) {
        for (int i = 0; i < stop_s; i++) {
            input.col(i) = input.col(i).array() / scale(i);
    ...
    Eigen::MatrixXd recon = input * V_.topRows(stop_s);
    recon.rowwise() += Xavg_;
    return recon;
}

int SpaceMapper::PCA_Dimension(const Eigen::MatrixXd &X, double threshold) {
    Eigen::RowVectorXd Xavg = X.colwise().mean();
    Eigen::MatrixXd Xp = X.rowwise() - Xavg;
    ...
    auto it = std::lower_bound(s_rev.begin(), s_rev.end(), threshold);
    int idx = static_cast<int>(it - s_rev.begin());
    return static_cast<int>(s.size()) - idx;
}

SpaceMapper SpaceMapper::Uncorrellated_Space(const Eigen::MatrixXd &X, bool enable_scale, double threshold, int dimension) {
    if (threshold != 1e-6 && dimension != -1 && threshold != 0.0) {
        if (dimension != -1) {
            throw std::runtime_error("Please only set one of threshold or dimension");
        }
    ...
    if (threshold != 0.0 && dimension == -1) {
        ...
        auto it = std::lower_bound(s_rev.begin(), s_rev.end(), threshold);
        int idx = static_cast<int>(it - s_rev.begin());
        stop_s = static_cast<int>(s.size()) - idx;
    } else if (dimension != -1) {
        stop_s = dimension;
    } else {
        const Eigen::VectorXd &s = space_mapper.s_;
    ...
    if (enable_scale) {
        ...
        for (int i = 0; i < stop_s; i++) {
            ...
            double cmax = proj.col(i).maxCoeff();
            double denom = cmax - cmin;
            space_mapper.scale(i) = (denom == 0.0) ? 1.0 : 1.0 / denom;
        }
    } else {
        space_mapper.scale.resize(0);
    }
