...
def laplace_coord(pred, placeholders, block_id):
	# pad a zero row matching pred's column dimension
	dim = tf.shape(pred)[1]
	print("dim:", dim)
	pad = tf.zeros([1, dim], dtype=pred.dtype)
	vertex = tf.concat([pred, pad], axis=0)
	lape = placeholders['lape_idx'][block_id-1]
	indices = lape[:, :8]
	counts = tf.cast(lape[:, -1], tf.float32)
	counts = tf.where(tf.equal(counts, 0.), tf.ones_like(counts), counts)
	weights = tf.reshape(1.0 / counts, [-1,1])
	# mask out padding indices (-1)
	valid_mask = tf.cast(indices >= 0, tf.float32)
	indices_clipped = tf.maximum(indices, 0)
	gathered = tf.gather(vertex, indices_clipped)  # [N,8,3]
	weighted_sum = tf.reduce_sum(gathered * tf.expand_dims(valid_mask, -1), axis=1)
	laplace = pred - weighted_sum * weights
	return laplace
